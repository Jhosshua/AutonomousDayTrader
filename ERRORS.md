...
**What worked instead:** Use the adapted stop for sizing; let R targets anchor to the real fill; preserve VWAP's 0.50R band-or-fallback rule and Mean Reversion's 1.0R structural entry gate. Wait for 50 regular-session closes for VWAP and use regular-session volume for News. Link staged Swing orders to their local order id, settle before any retry, count pending slots, attach stop metadata on every fill path, submit emergency sells, and persist the last scan with its session date. Keep AMD reserved until a possibly live Swing buy at Alpaca is final. Preserve broker-linked orders during pruning while dropping completed orders when no retention slots remain. The News contradiction exit remains cross-strategy for intraday holdings and excludes Swing.

**Verification and limits:** Claude Code critiqued the plan and implementation in read-only mode. Focused broker tests cover no-fill, partial and late fills, restart, duplicate prevention, stop execution and the 16:00 slot count while Alpaca settlement remains unresolved. Full `pytest -q`: 899 passed; frontend build passed. The integrated 184-event replay passed with one protected runner at 10:30 ET. That replay is synthetic wiring evidence, not a live Swing or News trade. Market orders can still slip past a structural target after fill; the Mean Reversion path logs this and keeps its protective stop. Railway deployment `ccde6327` of commit `04a70d9` succeeded after the user's immediate-deploy instruction and a verified flat app/Alpaca book. Postrestart health, broker match, durable restore, and live API checks passed; no new live News or Swing fill is claimed.
