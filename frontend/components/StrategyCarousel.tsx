...
/** F: "The 4 ways it trades" - a 4-col grid on desktop, stacked single column on mobile. */
export default function StrategyCarousel({ strategies, ledgerByStrategy, showPro }: StrategyCarouselProps) {
  return (
    <section className="flex flex-col gap-4">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink">The 4 ways it trades</h2>
        <div className="hidden sm:block text-sm text-muted">Colored bar = hours it may trade. Dark line = now.</div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-[18px]">
        {strategies.map((s, i) => (
